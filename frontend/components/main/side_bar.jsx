    ...
    return (
        <nav className="sidebar-container" onClick={closeNav}>
            <section className="sidebar-top">
                <Link className="logo-link" to="/home"><img src={window.allamaLogoImgURL} alt="a llama logo"/></Link>
                <div className="sidebar-burger" id="close-sidebar" >&lt;close</div>
            </section>
            ...
            <section className="sidebar-team-list">
                {
                    props.team.map((member) => {
                        return(
                            <div key={member.id} className={`nav-avatar ${COLORS[member.id % 20]}`}>
                                {getInitials(member)}
                            </div>
                        )
                    })
